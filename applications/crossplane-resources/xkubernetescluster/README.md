...
- **`Workspace.spec.forProvider.source: Inline`, not `Remote`.** Decided when `flux-platform-src` was believed to be private, which would have meant a git-sourced Workspace needing its own unproven git-clone credential wired into `provider-terraform`. **CORRECTED 2026-08-03**: the repo is actually public (confirmed via `gh repo view`), so `Remote` wouldn't need a credential at all — reconsideration flagged in `docs/memory/m4-step-tracker.md`, not yet acted on since this Workspace already provisions live VMs and switching `source` in place needs a careful state-impact plan first. The Composition embeds `providers/kvm/modules/talos-cluster`'s current `.tf` content directly for now — **keep the embedded copy and the real module in sync by hand**; this is a known, accepted drift risk, not an oversight.
