...
- [Node GUA On-Link Reply Unreliable](node-gua-onlink-reply-unreliable.md) — GUA-sourced VLAN-100 client traffic to a fleet VIP drops mid-handshake every time; ULA-sourced traffic works reliably (proven fix direction: internal Gateway + split-horizon DNS); blocks VLAN 179 final cutover
