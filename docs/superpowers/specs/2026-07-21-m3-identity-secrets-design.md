...
### A7 — Keycloak realm and group model

Realm: `ryezone-labs`.

Per-app group model (each app gets its own groups; no shared `platform-admin`
catch-all). Groups become OIDC claims asserted by Pinniped and consumed by
each app's own RBAC/role mapping.

**Groups bootstrapped at M3:**

| Group | Binds to |
|---|---|
| `k8s-admin` | ClusterRole `cluster-admin` via Pinniped RBAC |
| `k8s-viewer` | ClusterRole `view` via Pinniped RBAC |
| `keycloak-admin` | Keycloak realm-admin composite role (group→role mapping in declarative realm config); allows OIDC login to the Keycloak admin console |
| `openbao-admin` | OpenBao `admin` policy — full vault management (secrets, policies, auth methods, `sys/`) |
| `openbao-operator` | OpenBao `operator` policy — `read`/`list` on `secret/*`; no `sys/` or policy management; the read-only service tier |

**Keycloak local admin credential**: created at first-boot via declarative
config; stored in OpenBao at `kv/platform/keycloak/local-admin` as break-glass.
Required because if Keycloak is unhealthy, the OIDC flow that `keycloak-admin`
depends on won't work — the local credential is the recovery path.

**Reserved for future milestones** (not created at M3):

| Groups | Milestone |
|---|---|
| `grafana-admin`, `grafana-editor`, `grafana-viewer` | M5 |
| `backstage-admin` | later |
| `dispatch-admin`, `dispatch-responder` | M10 |
| `nats-admin` | M10 |
| `tenant-<name>-admin`, `tenant-<name>-dev` | M9, auto-generated by XTenant |
...
| 8 | `keycloak-db` CNPG cluster on `democratic-csi-nfs-pg`; barman to Garage `keycloak-db-barman` bucket | | keycloak-db CNPG Cluster Ready; barman backup active |
...
| 9b | Keycloak: deploy on `controlplane`, wire to keycloak-db; realm `ryezone-labs` + groups bootstrapped declaratively (see A7); expose at `id.rye.ninja` via Envoy Gateway HTTPS terminate + Let's Encrypt cert (A5); local admin credential stored in OpenBao as break-glass | | Keycloak admin UI reachable at `https://id.rye.ninja` with a browser-trusted cert; realm and all M3 groups exist; declarative config re-applies cleanly |
...
- ~~**[H] Realm/group names**~~: Closed — see A7. Per-app group model; five groups
  at M3 (`k8s-admin`, `k8s-viewer`, `keycloak-admin`, `openbao-admin`,
  `openbao-operator`); local admin credential in OpenBao as break-glass.
