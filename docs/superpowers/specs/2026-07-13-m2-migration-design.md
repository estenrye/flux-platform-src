...
|---|---|---|---|
| A1 | "root identity must not change — fleet trust depends on it" | Live trust domain is `cluster.local` (ADR-16 drift), so every Roles Anywhere trust anchor and ABAC condition re-enrolls regardless; the root was already rotated ~28d before 2026-07-11 without fleet impact; current root expires **2026-10-02**, forcing another rotation mid-M4/M5 | **Mint a fresh long-lived root on `controlplane`.** The trust surface today is only Spot (being deleted) and workstation bootstraps. This is the cheapest rotation the fleet will ever have; it removes the riskiest move-state item and the October forced rotation |
...
The inventory's "MUST NOT change" section is superseded by A1. The new
invariants: fresh root generated offline (`step` CLI), key material
SOPS-encrypted under `clusters/controlplane/` per ADR-15; **new fingerprint
recorded in `tests/platform-baseline/values/controlplane.env`, in
`docs/memory/step-ca-connectivity-validation.md` (rewritten for
`ca.rye.ninja`), and in the migration ADR**. Root validity: 10 years,
intermediate 1 year with the ADR-15 quarterly rotation drill unchanged.
...
2. step-ca deployed per the existing `applications/step-ca/` layout with a
   `controlplane` variant; root/intermediate injected from SOPS; DNS names
...
first SVID is issued** (ADR-16). Implementation must find and fix the cause
...
| R2: `truenas-iscsi` still blocked on IPv6 (upstream csi-lib-iscsi); step-ca-db lands on NFS | Postgres-on-NFS is acceptable at this size (single small DB, dump backups + NAS ZFS snapshots); move PVCs to iSCSI when unblocked (tracked in [[talos-iscsi-truenas-csi]]); do not block M2 on it |
