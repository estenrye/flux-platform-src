...
|---|---|---|---|
| 2026-07-24 | init (first-ever) | openbao-0, openbao-1, openbao-2 | Pass — all 3 report `Sealed: false`, `Initialized: true`, `Total Shares: 5`/`Threshold: 3`, `Storage Type: postgresql`, `HA Enabled: true`. `openbao-0` active, `openbao-1`/`openbao-2` standby. Key shares + root token SOPS-encrypted. Step 8 (audit log) hit the `bao audit enable` API-removal error below on first attempt — fixed by declaring the audit device in `values.yaml` instead (#119). |
| 2026-07-24 | restart (config roll for the audit-device fix) | openbao-0, openbao-1, openbao-2 | Pass — all 3 pods cycled and re-unsealed cleanly, HA re-formed identically (`openbao-0` active, others standby). Confirmed via `kubectl logs`: `core: adding new audit device: path=to-stdout/` and `core: enabled audit backend: path=to-stdout/ type=file` on the active node, plus a live JSON audit entry. Audit-to-stdout is fully working end-to-end. |
| 2026-08-24 | restart (KVM host `mf-ms-a2-01` kernel upgrade `5.15.0-187`→`5.15.0-190`, full graceful VM power-cycle of all 10 fleet VMs) | openbao-0, openbao-1, openbao-2 | Pass — `openbao-1`/`openbao-2` came back stuck in `Completed` (exit 0, not auto-restarted despite `restartPolicy: Always`; root cause not investigated, just deleted and let the StatefulSet recreate them). `openbao-db` CNPG cluster took ~2 min to reach `Cluster in healthy state` (3/3) after the reboot — waited for that before unsealing, since storage type is `postgresql`. `OrderedReady` confirmed live: `openbao-1` was not created until `openbao-0` was unsealed and Ready, same for `openbao-2`/`openbao-1`. All 3 unsealed cleanly with the existing 3-of-5 shares, HA re-formed identically (`openbao-0` active, others standby). |
