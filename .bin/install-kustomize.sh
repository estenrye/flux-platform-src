...
download_hash() {
    ...
    HASH_EXPECTED=$(jq '.assets[] | select(.name == "kustomize_v'${VERSION_KUSTOMIZE}'_'${OS}'_'${ARCH}'.tar.gz") | .digest | split(":")[1]' -r "${TMP_HASH}")
    info "Expected hash: ${HASH_EXPECTED}"
}

# Download binary from Github URL
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/${TAG_NAME}/kustomize_v${VERSION_KUSTOMIZE}_${OS}_${ARCH}.tar.gz"
...
get_release_version() {
    ...
    if [[ -n "${VERSION_KUSTOMIZE}" ]]; then
      TAG_NAME="${TAG_PREFIX}${VERSION_KUSTOMIZE}"
    else
      info "Fetching latest kustomize release version"
      ...
      VERSION_KUSTOMIZE=`jq '[ .[] | select(.tag_name | startswith("'$TAG_PREFIX'")) ] | first | .tag_name | sub("'$TAG_PREFIX'"; "")' -r "${TMP_METADATA}"`
    fi
    
    info "Found tag name: ${TAG_NAME}"

    if [[ -n "${VERSION_KUSTOMIZE}" ]]; then
        info "Using ${VERSION_KUSTOMIZE} as release"
    else
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing kustomize to ${BIN_DIR}/kustomize"
    tar -xzof "${TMP_BIN}" -C "${TMP_DIR}"

    local CMD_MOVE="mv -f \"${TMP_DIR}/kustomize\" \"${BIN_DIR}\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"
