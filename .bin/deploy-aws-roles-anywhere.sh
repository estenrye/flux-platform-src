...
# Emit a ConfigMap of the real ARNs so clusters/${CLUSTER}/resources/
# flux.crossplane-providers.kustomization.yaml can inject them via Flux's
# postBuild.substituteFrom, instead of the DeploymentRuntimeConfig files
# hardcoding cluster-specific ARNs (the previous approach — found 2026-07-20
# pointing at a stale pre-generalization trust anchor with no easy way to
# tell the two apart at a glance). This file is committed: it's just ARNs,
# not credentials.
export TRUST_ANCHOR_ARN=$(get_iam_policy_arn "${CLUSTER}-Trust-Anchor" "TrustAnchorArn")
export IAM_ADMIN_PROFILE_ARN=$(get_iam_policy_arn "${CLUSTER}-IAM-Admin" "RolesAnywhereProfileArn")
export IAM_ADMIN_ROLE_ARN=$(get_iam_policy_arn "${CLUSTER}-IAM-Admin" "IAMRoleArn")
export ROUTE53_ADMIN_PROFILE_ARN=$(get_iam_policy_arn "${CLUSTER}-Route53-Admin" "RolesAnywhereProfileArn")
export ROUTE53_ADMIN_ROLE_ARN=$(get_iam_policy_arn "${CLUSTER}-Route53-Admin" "IAMRoleArn")
export RA_ADMIN_PROFILE_ARN=$(get_iam_policy_arn "${CLUSTER}-RA-Admin" "RolesAnywhereProfileArn")
export RA_ADMIN_ROLE_ARN=$(get_iam_policy_arn "${CLUSTER}-RA-Admin" "IAMRoleArn")

mkdir -p "clusters/${CLUSTER}/resources"
cat > "clusters/${CLUSTER}/resources/roles-anywhere-arns.yaml" <<EOF
# Generated by .bin/deploy-aws-roles-anywhere.sh — do not hand-edit, rerun
# the script instead. Consumed by flux.crossplane-providers.kustomization.yaml
# via spec.postBuild.substituteFrom; must live in flux-system (same
# namespace as the Kustomization referencing it, per the Flux API).
apiVersion: v1
kind: ConfigMap
metadata:
  name: roles-anywhere-arns
  namespace: flux-system
data:
  AWS_ROLESANYWHERE_TRUST_ANCHOR_ARN: "${TRUST_ANCHOR_ARN}"
  AWS_IAM_ADMIN_PROFILE_ARN: "${IAM_ADMIN_PROFILE_ARN}"
  AWS_IAM_ADMIN_ROLE_ARN: "${IAM_ADMIN_ROLE_ARN}"
  AWS_ROUTE53_ADMIN_PROFILE_ARN: "${ROUTE53_ADMIN_PROFILE_ARN}"
  AWS_ROUTE53_ADMIN_ROLE_ARN: "${ROUTE53_ADMIN_ROLE_ARN}"
  AWS_ROLESANYWHERE_ADMIN_PROFILE_ARN: "${RA_ADMIN_PROFILE_ARN}"
  AWS_ROLESANYWHERE_ADMIN_ROLE_ARN: "${RA_ADMIN_ROLE_ARN}"
EOF

echo "Wrote clusters/${CLUSTER}/resources/roles-anywhere-arns.yaml — review, add to"
echo "clusters/${CLUSTER}/kustomization.yaml if not already present, and commit."
