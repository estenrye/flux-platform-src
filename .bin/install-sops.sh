...
setup_tmp() {
    TMP_DIR=$(mktemp -d -t sops-install.XXXXXXXXXX)
    TMP_METADATA="${TMP_DIR}/sops.json"
    TMP_HASH="${TMP_DIR}/sops.hash"
    TMP_BIN="${TMP_DIR}/sops"
    cleanup() {
        local code=$?
        set +e
        trap - EXIT
...
download_hash() {
    ASSET_URL="https://api.github.com/repos/${GITHUB_REPO}/releases/tags/v${VERSION_SOPS}"

    info "Downloading hash ${ASSET_URL}"
    download "${TMP_HASH}" "${ASSET_URL}"

    HASH_EXPECTED=$(jq '.assets[] | select(.name == "sops-v'${VERSION_SOPS}'.'${OS}'.'${ARCH}'") | .digest | split(":")[1]' -r "${TMP_HASH}")
    info "Expected hash: ${HASH_EXPECTED}"
}

# Download binary from Github URL
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION_SOPS}/sops-v${VERSION_SOPS}.${OS}.${ARCH}"
...
get_release_version() {
    if [[ -n "${SOPS_VERSION}" ]]; then
      SUFFIX_URL="tags/v${SOPS_VERSION}"
    else
      SUFFIX_URL="latest"
    fi
    ...
    VERSION_SOPS=$(grep '"tag_name":' "${TMP_METADATA}" | sed -E 's/.*"([^"]+)".*/\1/' | cut -c 2-)
    if [[ -n "${VERSION_SOPS}" ]]; then
        info "Using ${VERSION_SOPS} as release"
    else
...
setup_binary() {
    ...
    local CMD_MOVE="mv -f \"${TMP_BIN}\" \"${BIN_DIR}\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"
