...
GITHUB_REPO="rackspace-spot/spotctl"
TAG_PREFIX="v"
TMP_PREFIX="spotctl"
...
download_hash() {
    ...
    HASH_EXPECTED=$(jq '.assets[] | select(.name == "spotctl-'${OS}'-'${ARCH}'") | .digest | split(":")[1]' -r "${TMP_HASH}")
    info "Expected hash: ${HASH_EXPECTED}"
}

# Download binary from Github URL
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/${TAG_NAME}/spotctl-${OS}-${ARCH}"
...
get_release_version() {
    ...
    if [[ -n "${VERSION_SPOTCTL}" ]]; then
      TAG_NAME="${TAG_PREFIX}${VERSION_SPOTCTL}"
    else
      info "Fetching latest spotctl release version"
      ...
      VERSION_SPOTCTL=`jq '[ .[] | select(.tag_name | startswith("'$TAG_PREFIX'")) ] | first | .tag_name | sub("'$TAG_PREFIX'"; "")' -r "${TMP_METADATA}"`
    fi
    
    info "Found tag name: ${TAG_NAME}"

    if [[ -n "${VERSION_SPOTCTL}" ]]; then
        info "Using ${VERSION_SPOTCTL} as release"
    else
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing spotctl to ${BIN_DIR}/spotctl"

    local CMD_MOVE="mv -f \"${TMP_BIN}\" \"${BIN_DIR}/spotctl\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"
