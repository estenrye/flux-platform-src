...
setup_tmp() {
    TMP_DIR=$(mktemp -d -t yq-install.XXXXXXXXXX)
    TMP_METADATA="${TMP_DIR}/yq.json"
    TMP_HASH="${TMP_DIR}/yq.hash"
    TMP_BIN="${TMP_DIR}/yq"
    cleanup() {
        local code=$?
        set +e
...
download_hash() {
    ASSET_URL="https://api.github.com/repos/${GITHUB_REPO}/releases/tags/v${VERSION_YQ}"

    info "Downloading hash ${ASSET_URL}"
    download "${TMP_HASH}" "${ASSET_URL}"

    HASH_EXPECTED=$(jq '.assets[] | select(.name == "yq_'${OS}'_'${ARCH}'") | .digest | split(":")[1]' -r "${TMP_HASH}")
    info "Expected hash: ${HASH_EXPECTED}"
}

# Download binary from Github URL
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION_YQ}/yq_${OS}_${ARCH}"
...
get_release_version() {
    ...
    VERSION_YQ=$(grep '"tag_name":' "${TMP_METADATA}" | sed -E 's/.*"([^"]+)".*/\1/' | cut -c 2-)
    if [[ -n "${VERSION_YQ}" ]]; then
        info "Using ${VERSION_YQ} as release"
    else
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing yq to ${BIN_DIR}/yq"
