...
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/${TAG_NAME}/chainsaw_${OS}_${ARCH}.tar.gz"
...
get_release_version() {
    ...
    if [[ -n "${VERSION_CHAINSAW}" ]]; then
      TAG_NAME="${TAG_PREFIX}${VERSION_CHAINSAW}"
    else
      info "Fetching latest chainsaw release version"
      ...
      VERSION_CHAINSAW=`jq '[ .[] | select(.tag_name | startswith("'$TAG_PREFIX'")) ] | first | .tag_name | sub("'$TAG_PREFIX'"; "")' -r "${TMP_METADATA}"`
    fi
    
    info "Found tag name: ${TAG_NAME}"

    if [[ -n "${VERSION_CHAINSAW}" ]]; then
        info "Using ${VERSION_CHAINSAW} as release"
    else
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing chainsaw to ${BIN_DIR}/chainsaw"
    tar -xzof "${TMP_BIN}" -C "${TMP_DIR}"

    local CMD_MOVE="mv -f \"${TMP_DIR}/chainsaw\" \"${BIN_DIR}\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"
