...
download_hash() {
    ...
    HASH_EXPECTED=$(jq '.assets[] | select(.name == "gh_'${VERSION_GH}'_'${OS}'_'${ARCH}'.zip") | .digest | split(":")[1]' -r "${TMP_HASH}")
    info "Expected hash: ${HASH_EXPECTED}"
}

# Download binary from Github URL
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/${TAG_NAME}/gh_${VERSION_GH}_${OS}_${ARCH}.zip"
...
get_release_version() {
    ...
    if [[ -n "${VERSION_GH}" ]]; then
      TAG_NAME="${TAG_PREFIX}${VERSION_GH}"
    else
      info "Fetching latest gh release version"
      ...
      VERSION_GH=`jq '[ .[] | select(.tag_name | startswith("'$TAG_PREFIX'")) ] | first | .tag_name | sub("'$TAG_PREFIX'"; "")' -r "${TMP_METADATA}"`
    fi
    
    info "Found tag name: ${TAG_NAME}"

    if [[ -n "${VERSION_GH}" ]]; then
        info "Using ${VERSION_GH} as release"
    else
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing gh to ${BIN_DIR}/gh"
    tar -xzof "${TMP_BIN}" -C "${TMP_DIR}"
    ls -l "${TMP_DIR}"
    local CMD_MOVE="mv -f \"${TMP_DIR}/gh_${VERSION_GH}_${OS}_${ARCH}\" \"${BIN_DIR}\""
    local CMD_LNK="ln -sf \"${BIN_DIR}/gh_${VERSION_GH}_${OS}_${ARCH}/bin/gh\" \"${BIN_DIR}/gh\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
        eval "${CMD_LNK}"
    else
