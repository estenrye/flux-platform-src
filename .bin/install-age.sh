...
setup_tmp() {
    TMP_DIR=$(mktemp -d -t age-install.XXXXXXXXXX)
    TMP_METADATA="${TMP_DIR}/age.json"
    TMP_HASH="${TMP_DIR}/age.hash"
    TMP_BIN="${TMP_DIR}/age.tar.gz"
    cleanup() {
        local code=$?
        set +e
        trap - EXIT
...
download_hash() {
    ASSET_URL="https://api.github.com/repos/${GITHUB_REPO}/releases/tags/v${VERSION_AGE}"

    info "Downloading hash ${ASSET_URL}"
    download "${TMP_HASH}" "${ASSET_URL}"

    HASH_EXPECTED=$(jq '.assets[] | select(.name == "age-v'${VERSION_AGE}'-'${OS}'-'${ARCH}'.tar.gz") | .digest | split(":")[1]' -r "${TMP_HASH}")
    info "Expected hash: ${HASH_EXPECTED}"
}

# Download binary from Github URL
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION_AGE}/age-v${VERSION_AGE}-${OS}-${ARCH}.tar.gz"
...
get_release_version() {
    ...
    VERSION_AGE=$(grep '"tag_name":' "${TMP_METADATA}" | sed -E 's/.*"([^"]+)".*/\1/' | cut -c 2-)
    if [[ -n "${VERSION_AGE}" ]]; then
        info "Using ${VERSION_AGE} as release"
    else
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing age to ${BIN_DIR}/age"
    tar -xzof "${TMP_BIN}" -C "${TMP_DIR}"

    local CMD_MOVE="mv -f \"${TMP_DIR}/age/age\" \"${TMP_DIR}/age/age-inspect\" \"${TMP_DIR}/age/age-keygen\" \"${TMP_DIR}/age/age-plugin-batchpass\" \"${BIN_DIR}\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"
