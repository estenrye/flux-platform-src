...
setup_tmp() {
    TMP_DIR=$(mktemp -d -t jq-install.XXXXXXXXXX)
    TMP_METADATA="${TMP_DIR}/jq.json"
    TMP_HASH="${TMP_DIR}/jq.hash"
    TMP_BIN="${TMP_DIR}/jq.tar.gz"
    cleanup() {
        local code=$?
        set +e
        trap - EXIT
...
download_hash() {
    ASSET_URL="https://api.github.com/repos/${GITHUB_REPO}/releases/tags/jq-${VERSION_JQ}"

    info "Downloading hash ${ASSET_URL}"
    download "${TMP_HASH}" "${ASSET_URL}"

    HASH_EXPECTED=$(grep -A 50 "\"name\": \"jq-${OS}-${ARCH}\"" "${TMP_HASH}" | grep -m1 '"digest"' | sed 's/.*"digest": *"[^:]*:\([^"]*\)".*/\1/')
    info "Expected hash: ${HASH_EXPECTED}"
}

# Download binary from Github URL
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/jq-${VERSION_JQ}/jq-${OS}-${ARCH}"
...
get_release_version() {
    ...
    VERSION_JQ=$(grep '"tag_name":' "${TMP_METADATA}" | sed -E 's/.*"([^"]+)".*/\1/' | cut -c 4-)
    if [[ -n "${VERSION_JQ}" ]]; then
        info "Using ${VERSION_JQ} as release"
    else
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing jq to ${BIN_DIR}/jq"

    local CMD_MOVE="mv -f \"${TMP_BIN}\" \"${BIN_DIR}/jq\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"
